...
- **Read the card before the code.** When a decision anywhere in this repository depends on what a card does — a parser pattern, an engine outcome, an AI verdict, a combo-detector class, a display, a test fixture, a plan's scope — the card's text is read first and the code is reconciled to it; the card's reading determines the way forward. Start from the Oracle text of the cards involved, derive the expected outcome clause by clause, then read the code to find where it diverges. A possibility the code admits is not a behavior until a card's text produces it, and a class enumerated from fields and match arms alone is not a finding until a real card instantiates it. When the code and the card disagree, the card is right and the code is the defect. This is the companion of "Verify the card, not just the rule": that bullet confirms the premise is real; this one keeps every decision that rests on it anchored to the card.
