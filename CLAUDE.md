...
- **Read the card before the code.** A card's text lines decide what the parser must produce and what the engine must do; the code's structure is what gets judged against them, never the reference. When reasoning about an outcome — which cards a veto or a bound must cover, what class a fix touches, whether a behavior is a defect — start from the Oracle text of the cards involved, derive the expected behavior clause by clause, then read the code to find where it diverges. A possibility the code admits is not a behavior until a card's text produces it, and a class enumerated from fields and match arms alone is not a finding until a real card instantiates it. When the code and the card disagree, the card is right and the code is the defect. This is the companion of "Verify the card, not just the rule": that bullet confirms the premise is real; this one keeps the reasoning anchored to it.
