...
/// window, from BOTH stores — the zone-gated object-attached ones
...
pub(crate) fn board_has_active_replacement_among(
    ...
    functioning_board_replacement_defs(state).any(|def| {
        ...
        || live_floating_replacement_defs(state).any(|def| repl_events.contains(&def.event))
}
