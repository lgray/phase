    ...
    if !target_slots.is_empty() {
        ...
        if let Some(targets) = resolved_targets {
            ...
            if !triggers::mutate_pending_trigger_entry(state, &trigger.ability) {
                ...
                // prompt could open. Recover per CR 608.1 (only the spell or
                // ability on the stack resolves, so an entry that has left it
                // cannot) — record the diagnostic, abandon, return priority
                // (re-normalized next pass; CR 117.3b would give the active
                // player).
                triggers::restore_trigger_event_context(state, mode_context_snapshot);
        ...
        if !triggers::finalize_pending_trigger_entry(state, &trigger.ability) {
            ...
            // Recover per CR 608.1 (only the spell or ability on the stack
            // resolves, so an entry that has left it cannot) — record the
            // diagnostic, abandon, and hand back priority instead of panicking
            // (re-normalized next pass; CR 117.3b would give the active player).
            triggers::abandon_ceased_pending_trigger(state, &trigger.ability);
