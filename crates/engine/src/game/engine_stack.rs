...
pub(super) fn finalize_trigger_target_selection(
            ...
            match dist_targets.as_slice() {
                ...
                _ => {
                    ...
                    if !triggers::mutate_pending_trigger_entry(state, &trigger.ability) {
                        ...
                        // division prompt could open. Recover per CR 608.1 (only
                        // the spell or ability on the stack resolves, so an entry
                        // that has left it cannot) — record the diagnostic,
                        // abandon, hand back priority. Matches the
                        // DistributeAmong-return convention below; the next
                        // priority pass re-normalizes (CR 117.3b would give the
                        // active player).
    ...
    if !triggers::finalize_pending_trigger_entry(state, &trigger.ability) {
        ...
        // Recover per CR 608.1 (only the spell or ability on the stack resolves,
        // so an entry that has left it cannot) — record the diagnostic, abandon
        // the dead trigger, and hand control back rather than panic. Returns
        // Priority for the controller (matching the DistributeAmong convention
        // above); the next priority pass re-normalizes (CR 117.3b would give
        // active player).
        triggers::abandon_ceased_pending_trigger(state, &trigger.ability);
        priority::clear_priority_passes(state);
        return WaitingFor::Priority { player: controller };
