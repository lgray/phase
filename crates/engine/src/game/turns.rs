...
pub fn start_next_turn(state: &mut GameState, events: &mut Vec<GameEvent>) {
    ...
    // CR 400.7: defence in depth for the two ledgers above. A parked token battlefield entry is
    // realized within the action that settles, and every prompt-abandonment path clears it, so none
    // should reach a turn boundary. One that did would write its row onto the NEXT turn's freshly
    // cleared ledger — an "entered this turn" answer for an entry that happened last turn. Mirrors
    // the `deferred_entry_events` clears in `elimination.rs` / `scenario_db.rs`.
