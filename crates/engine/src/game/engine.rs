    ...
    fn the_cr_603_5_prompt_census_is_pinned_so_a_sixth_producer_is_a_counted_event() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                // ⚠ RE-REBASE onto upstream `7127326673`: `:12038 ⇒ :12043`, located by content
                // digest, offset from `begin_pending_trigger_target_selection` unchanged at 134.
                // ⚠ RE-REBASE onto upstream `635c51ec4` (#7382, pre-entry opponent controller):
                // `:12043 ⇒ :12047`, +4 entirely above this producer. MEASURED in the rebased file,
                // not computed: the enclosing-fn offset is the control and is STILL 134, which is
                // what re-establishes producer identity — the same mint text appears at several
                // coordinates in this crate, so the offset discriminates where the text cannot.
                // This rebase surfaced as a CONFLICT in this very literal, which is the drift class
                // FU-4 (content-hash coordinate anchor) exists to end; logged there, not re-argued here.
                "game/engine.rs:12047".to_string(),
            ],
