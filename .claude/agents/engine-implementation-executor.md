...
5. The externally owned canonical receipt destination — a receipt file path (v1) or a `receipt.json` the orchestrator attaches as a git note under `refs/notes/engine-implementer` (v2; the note and its ref are part of the canonical receipt, not a parallel record) — plus the read-only pinned data root containing `AtomicCards.json` and completion evidence destinations. The receipt is the only provenance contract; do not create a manifest, seal, provenance envelope, replica/quorum report, or parallel parser ledger beyond it.
...
Everything else — the receipt contract, multi-agent safety, nom mandate, CR verification, preparatory verification blocks, and the stable-HEAD rules — is unchanged in phase mode.
