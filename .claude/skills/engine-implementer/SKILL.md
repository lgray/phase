...
### Receipt v2 — git-native evidence records

`format=engine-implementer-receipt-v2` replaces v1's 520-line `key=value` document with git itself. Git already proves identity (the note is attached to the candidate commit), history (the side branch chains round to round — read SHAs and diffs from git, never restate them), and integrity (editing the note changes its git OID). The receipt records only what git cannot know: what was run and what came back. A run picks one format at start and keeps it; an in-flight v1 run finishes on v1.

The receipt is one `receipt.json` per candidate, attached as a git note by the orchestrator after completion verification and pushed with the branch:

```bash
git notes --ref=refs/notes/engine-implementer add -F receipt.json "$CANDIDATE_SHA"
git push origin refs/notes/engine-implementer
```

It may be amended (`git notes ... add -f`) only until the candidate is accepted. Contents — plain JSON, nothing more:

```json
{
  "format": "engine-implementer-receipt-v2",
  "run": "windfall-1",
  "phase": 3,
  "scope": ["crates/engine/src/analysis/resource.rs"],
  "projection": null,
  "checks": [
    {"name": "test-engine", "cmd": "<exact command>", "exit": 0,
     "full_suite": true, "binary_sha256": "<sha256 of the executable, else null>"}
  ]
}
```

`run` is the run's process-records directory name under `<git-common-dir>/engine-implementer-runs/`. `phase` is the phase index, or `null` for an unphased run. `scope` is the frozen scope-path list. `projection` is `null` when no projection was required; otherwise `{"reason": "<why>", "steps": [...]}` where each step is a row shaped like a check (`name`, `cmd`, `exit`, plus `outputs`, a list of `{path, size, sha256}` covering every file that step wrote) recording the projection build, generator, report, and comparator commands from the projection procedure earlier in this skill. Each check records its name, the exact command, the exit code, whether it ran unfiltered (`full_suite`), and `binary_sha256` — the SHA256 of the single executable the command built and ran (the path cargo prints), or `null` when the command builds none or no single executable is identifiable. Check names are unique within a receipt and stay the same across rounds so rounds can be compared; a name that is duplicated, missing on one side, or renamed makes that comparison `CANNOT_ANSWER`, never a silent skip. `full_suite` is `false` for any filtered or narrowed run — a filtered run recorded without that flag is indistinguishable from a suite pass, which is a false green.

**What the reviewer does with it (Step 6):** read the note (`git notes --ref=refs/notes/engine-implementer show "$CANDIDATE_SHA"`), take every SHA, diff, and round relationship from git itself (`BASE_SHA`/`CANDIDATE_SHA` and the frozen scope-path list arrive as spawn inputs; the previous candidate is the first commit in `git rev-list "$CANDIDATE_SHA"^` whose note carries the same `run` and `phase` — one notes ref serves every run, so a note alone is not a match; none in that history means a first round), re-run whatever it needs to trust, and quote the note's git OID (`git notes --ref=refs/notes/engine-implementer list "$CANDIDATE_SHA"`) in its result as the receipt identity — that OID is the tamper evidence; the note never contains its own hash. The red flags to check every round: (1) when the review gates acceptance, every check must have `full_suite: true` — the orchestrator's brief states whether it does; (2) when the diff from the previous candidate touches a build input — the paths `scripts/engine-source-hash.sh` hashes (`crates/engine/src`, `crates/engine/data`, `crates/engine/build.rs`, `crates/engine/Cargo.toml`, `Cargo.lock`), plus any other `crates/` source in scope, `.cargo/config.toml`, and `rust-toolchain.toml` — compare each check's `binary_sha256` with the previous candidate's note and report any that did not change — an unchanged binary after a code change means the build was stale, and one stale binary hiding behind one rebuilt one must be named, not inferred from silence; checks whose `binary_sha256` is `null` on either side are exempt from that comparison; (3) the note's `scope` must equal the frozen scope-path list from the spawn inputs — a scope the note asserts about itself authorizes nothing. If the reviewer is missing anything it needs — the note, the previous note, whether this review gates acceptance — it says so plainly in its result instead of skipping the check: a silently skipped check reads as a pass from the outside, which is worse than a failure.
...
Two measured build-economy rules govern measurement worktrees. **Build-once directories never pay for incremental state:** every measurement build — projection, completion, any target directory built once and never rebuilt — runs with `CARGO_INCREMENTAL=0` (measured: dead incremental state was 17 of 28 GB on one completion target). **Completion allocation is per run-segment, not per candidate, in v2 runs:** one completion worktree and one isolated completion target directory per phase (per run when unphased), reset to each round's `CANDIDATE_SHA`, instead of fresh ones per candidate (measured per-candidate cost: seven cold builds, 130 GB, one phase). Reuse is strictly sequential within the owning run — the metadata-hash collision hazard is concurrent writers on one target path, which sequential reuse never creates; never share a reused directory with any concurrently running agent or lane. Recording each check's `binary_sha256` in the receipt note is what makes reuse safe: the reviewer compares hashes across rounds, so a reused directory that silently skips a rebuild is visible (Receipt v2 section). Every detached-identity, `HEAD`, and clean check at round start and end is unchanged — reuse changes allocation, never what is measured. A reused completion directory is no longer build-once, so the first rule's rationale does not carry to it automatically; it still runs `CARGO_INCREMENTAL=0` — the 17 GB disk cost is measured, the warm-rebuild speedup incremental state might buy is not — and the first v2 run owes a re-measurement if warm completion rebuilds dominate its wall clock. v1 runs allocate per candidate as before: v1 cannot record binary hashes, so a v1 run must never reuse a completion directory.
...
**Completion tiering (v2 chartered runs).** The full unfiltered suite is owed at the tree being shipped — the phase-final candidate. Today this repository squash-merges, so that is the only tree that lands; if the merge policy ever changes, the rule stays the same, because per-commit test runs are slow and tell you little — the shipping tip gates, intermediate commits do not. An intermediate fix-round candidate may run a narrowed check set — tests filtered to the touched surface, clippy to the changed crates — recorded with `full_suite: false` and the exact filter visible in each check's `cmd`. A narrowed round is never citable as a suite pass. Before any per-phase or run-level acceptance, re-run completion unfiltered on the same unchanged candidate, update the note, and have Step 6 re-review it; the new review's note OID is the one acceptance binds (this costs one completion pass and one review, not a fix round). Unphased and v1 runs always run the full suite. The Markdown-only carve-out above is orthogonal — it narrows what a completion checks by content class; tiering narrows when the full suite is owed by candidate role.
...
For a v2 receipt the reviewer follows the Receipt v2 section: SHAs and diffs from git, the note read and its git OID quoted as the receipt identity, the full-suite and stale-binary checks run every round. The orchestrator's brief must say whether this review gates acceptance — that is not derivable from the note. Anything the reviewer is missing, it reports plainly in its result rather than skipping the check.
