...
- **Review:** stop after **three unsuccessful review attempts within one review loop**, or when the **task-wide review allowance** is exhausted, whichever comes first. Each review loop carries its own allowance of three: the sizing-audit loop, the charter loop, each phase's (or the unphased run's) plan loop and implementation-review loop, and the integration loop. A loop restarted or renamed for the same work keeps that loop's tally. A phase added, split, merged or re-scoped by a decision revision starts its own loops at zero; that revision is counted in the charter loop. The task-wide allowance is fixed once — at the charter's first freeze in a chartered run, otherwise at the scope-freeze before the first executor dispatch — as three for every review loop entered before that freeze, plus three for the implementation-review loop (unphased) or for each frozen phase's plan and implementation-review loops and the integration loop (chartered); no later freeze or revision re-fixes it. Every unsuccessful attempt in any loop, before or after the freeze and in loops a later decision revision creates, draws it down; no loop, phase, revision or restart replenishes it. Findings requiring a revision count as an unsuccessful attempt; abandoning a dispatched review also counts. New findings count just as repeated ones do. Clean intermediate reviews consume no attempt and erase none. Charter corrections that require no re-review remain clean; review-only revisions requiring a fresh review count. A Step 6 round whose every finding is a comment-only correction (see Step 6) consumes no attempt, even when the comment it corrects is the wording of an acceptance row. Returning from implementation to planning re-enters that phase's plan loop with the history that loop already has. Only accepting a candidate that implements part of the original requested product behavior resets every loop's tally; nothing but explicit user authorization (see **At a stop**) restores the task-wide allowance; a checkpoint, clean plan, helper-only phase or new reviewer resets neither. If tooling itself is the user's requested product, its accepted implementation qualifies.
...
T4 explains when decomposition may help; it does not authorize another dispatch. When the [run limits](#run-limits) stop the work, include this diagnosis and a proposed smaller scope in the handoff. Only explicit authorization to resume permits a new charter or restart. Unclassified process findings and spot findings still count toward that loop's review limit even when they do not contribute to T4.
...
The full suite is owed at the tree being shipped. An intermediate fix round may narrow to the touched surface — say so plainly when reporting it, since a narrowed run is not a suite pass — and re-run unfiltered before acceptance. A comment-only correction (Step 6) is not a fix round: the suite and every other instrument that does not read comments stand at the candidate they ran on, and only the bounded checks named there run at the corrected commit.
...
**Comment-only corrections.** A finding is a comment-only correction when closing it adds or removes no non-comment line — a false, stale or missing sentence in a `//`, `///` or `//!` comment, a citation, a label — and the finding supplies the replacement text or the repair is deleting the false sentence. When every finding of a round is one, the round is a correction round, not a fix round: no executor is dispatched, no delta review follows, and the round consumes no attempt. The orchestrator applies each correction itself, inside the phase's `SCOPE_PATHS` (a comment-only edit outside the list follows the scope rule's standing class), as check-and-replace with two-sided verification (the old text present at the named coordinate before, the new text present exactly once after), commits once on the reviewed candidate, and records the class and the before/after text in the phase-fit record, as for a charter correction. The commit is proven comment-only by the diff: every non-blank `^[+-]` line is comment text, with a control that shows the same test rejects a code line. The checks at the corrected commit are the bounded set: `cargo fmt --all --check` always; the generated-bindings comparison when a `///` or `//!` line on a binding-exported type changed, because CI byte-compares the pair; clippy on the touched crates only when a `///` or `//!` line changed, because the default doc lints fail under `-D warnings`. The suite, the walk, the parser gate, coverage and the semantic audit are not run: none reads comments, and their results at the reviewed candidate stand. Each correction gets one direct instrument check with its control leg for the claim the finding named — the census or grep that flagged the sentence no longer returns it, and did at the reviewed candidate. A finding that needs a non-comment line, that supplies no replacement text and is not a deletion, or whose diff proof fails, is an ordinary fix round; when in doubt, it is a fix round. A correction round mixed with ordinary findings dispatches the fix round and folds the corrections into it as constraints.

### Step 7 — Final acceptance

Accept when the plan-review loop is clean, the review returns no findings or only comment-only corrections applied and proven as Step 6 says, the completion checks pass at the candidate (the bounded set at a corrected commit), and `rev-parse HEAD == CANDIDATE_SHA`, the corrected commit when one exists. In a chartered run this is per-phase acceptance, with `PHASE_BASE_SHA` substituted; it emits no Final Report snapshot and no PR handoff, which are run-level only.
